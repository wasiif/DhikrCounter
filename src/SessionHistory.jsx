function SessionHistory({ history, onClearHistory }) {
  ...
  return (
    <section className="panel panel-history" id="history">
      ...
      {history.length === 0 ? (
        <p className="empty-message">No completed sessions yet. Complete a session to save it here.</p>
      ) : (
            ...
            <ul>
              {sessions.map((session) => (
                <li key={session.id} className="history-row">
                  <div>
                    <strong>{session.phrase}</strong>
                    <p>{session.count} repetitions · {session.durationMinutes} min</p>
                  </div>
